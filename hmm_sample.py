...
class HMM(object):
    def __init__(self, initial_prob, trans_prob, obs_prob):
        ...
        self.inital_prob = initial_prob
        self.trans_prob = trans_prob
        self.emission = tf.constant(obs_prob)

        assert self.inital_prob.shape == (self.N, 1)
    ...
    def get_emission(self, obs_idx):
        """ helper function for accessing obserbation probabilities """
    ...
    def forward_init_op(self):
        ...
        fwd = tf.multiply(self.inital_prob, obs_prob)

        return fwd

    def forward_op(self):
        """ update cache """
        trainsitions = tf.matmul(
            self.fwd, tf.transpose(self.get_emission(self.obs_idx))
        )
        weighted_transitions = trainsitions * self.trans_prob
    ...
    def decode_op(self):
        transitions = tf.matmul(
            self.viterbi, tf.transpose(self.get_emission(self.obs_idx))
        )
...
def forward_algorithm(sess, hmm, observations):
    fwd = sess.run(
        hmm.forward_init_op(),
        feed_dict={hmm.obs_idx: observations[0]}
    )
...
def viterbi_decode(sess, hmm, observations):
    ...
    for t in range(1, len(observations)):
        viterbi, backpt = sess.run(
            [hmm.decode_op(), hmm.backpt_op()],
            feed_dict={
                hmm.obs_idx: observations[t],
                hmm.viterbi: viterbi,
            },
